                ...
                <section class="card">
                    ...
                    <div class="card-body">
                        <form id="myForm" class="form-horizontal form-bordered" method="post"
                              action="{{url('/admin/brand_edit/'.$brand->id)}}">
                            {{csrf_field()}}
                                ...
                                <div class="col-lg-6">
                                    <div class="input-group">
                                        <input hidden type="text" name="last_loqo" class="form-control"
                                               value="{{base64_encode($brand->logo)}}"/>
                                        <img style='display:block;width:80px;height:60px;'
                                             src="data:image/jpeg;base64,{{base64_encode($brand->logo)}}"/>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-lg-6">
                                    <div class="input-group">
                                        ...
                                        <select name="status" data-plugin-selectTwo class="form-control populate">
                                            <option value="1" @if( $brand->status==1) selected @endif>Aktiv</option>
                                            <option value="0" @if( $brand->status==0) selected @endif>Deaktiv</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
